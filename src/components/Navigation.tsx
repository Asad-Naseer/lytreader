...
import { uploadToDropbox, downloadFromDropbox, getAccessToken, startDropboxAuth } from '../utils/dropbox';
...
function Navigation({ setBooks }: { setBooks: Function }) {
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                ...
                const newBookObj: Book = { id: bookId, name: file.name, cover: permanentCover || '', data: arrayBuffer };
    ...
    const handleSyncFromCloud = async () => {
        ...
        if (json) {
            ...
            window.location.reload();
        } else { alert("Cloud backup not found."); }
    ...
    return (
        <nav className="ma2 br3 ba bw1 navBorderColor bg-[#1c1c1c] flex flex-wrap lg:flex-nowrap items-center justify-center lg:justify-between p-2 md:ph4 md:pv2 gap-4 shadow-lg">
            ...
            <input type='file' ref={restoreInputRef} style={{ display: 'none' }} onChange={handleLocalRestore} accept=".json" />

            {/* LEFT GROUP: Library Management */}
            <div className="flex items-center justify-center gap-2 w-full sm:w-auto">
                <button className="flex-1 sm:flex-none text-gray-200 bg-white/5 hover:bg-white/10 bn pointer pv2 ph3 br2 transition-all flex items-center justify-center gap-2 text-sm font-medium whitespace-nowrap" onClick={() => fileInputRef.current?.click()}>
                    <span>📖</span> <span className="dn di-m di-l">Import</span> Books
                </button>
                <button className="flex-1 sm:flex-none text-gray-200 bg-white/5 hover:bg-white/10 bn pointer pv2 ph3 br2 transition-all flex items-center justify-center gap-2 text-sm font-medium whitespace-nowrap" onClick={() => folderInputRef.current?.click()}>
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 w-full lg:w-auto">
                ...
                <div className="flex items-center justify-center gap-2 w-full sm:w-auto">
                    {!isLoggedInDropbox ? (
                        <button className="flex-1 sm:flex-none bg-indigo-700 hover:bg-indigo-600 white bn pointer pv2 ph3 br2 transition-colors text-xs font-bold uppercase tracking-tight whitespace-nowrap" onClick={startDropboxAuth}>
                            🔗 Connect Dropbox
                        </button>
                    ) : (
                        <div className="flex items-center justify-center gap-2 w-full sm:w-auto">
                            <button disabled={isToCloudLoading || isFromCloudLoading} className="flex-1 sm:flex-none bg-indigo-700 hover:bg-indigo-600 white bn pointer pv2 ph3 br2 transition-colors text-xs font-bold uppercase tracking-tight min-w-[120px] whitespace-nowrap" onClick={handleSyncToCloud}>
                                {isToCloudLoading ? '⏳...' : '☁️ Sync To Cloud'}
                            </button>
                            <button disabled={isToCloudLoading || isFromCloudLoading} className="flex-1 sm:flex-none bg-teal-700 hover:bg-teal-600 white bn pointer pv2 ph3 br2 transition-colors text-xs font-bold uppercase tracking-tight min-w-[140px] whitespace-nowrap" onClick={handleSyncFromCloud}>
                ...
                <div className="flex items-center justify-center gap-2 w-full sm:w-auto border-t border-white/5 pt-3 sm:pt-0 sm:border-t-0">
                    <button className="flex-1 sm:flex-none bg-blue-700 hover:bg-blue-600 white bn pointer pv2 ph3 br2 transition-colors text-xs font-bold uppercase tracking-tight whitespace-nowrap" onClick={handleLocalExport}>
                        📤 Export
                    </button>
                    <button className="flex-1 sm:flex-none bg-red-800 hover:bg-red-700 white bn pointer pv2 ph3 br2 transition-colors text-xs font-bold uppercase tracking-tight whitespace-nowrap" onClick={() => restoreInputRef.current?.click()}>
                        📥 Restore
                    </button>
                </div>
            </div>
